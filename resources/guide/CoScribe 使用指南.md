...
- 项目隐藏文件 `.coscribe/COSCRIBE.md` 用于保存稳定的项目目标、术语、偏好和约束；旧根目录记忆会在首次打开时复制导入，原文件保持不变。
...
- 点击聊天工具栏中的“截图”，或按 `Cmd/Ctrl + Shift + D`。应用先保留原始物理分辨率的屏幕图像，再只显示透明、无边框的十字取景层；按住鼠标拖出需要的区域并松开确认，`Esc` 取消。最终附件从原始屏幕像素裁剪，不会用低清快照覆盖桌面，也不会自动识别或预选区域。
- macOS 首次截图会触发系统屏幕录制授权；权限已拒绝、受限或记录失效时，应用会自动打开“系统设置 → 隐私与安全性 → 屏幕录制”。如果列表中已有 CoScribe 但仍无画面，请删除旧条目，完全退出 App，从 Finder 重新打开当前版本后再授权。
