...
> `v4.0.0` 开发分支新增完整 IDE 工作区：左侧 IDE 入口只切换中央工作区，AI 侧栏保持不变；代码文件单击选择、双击打开，可编辑保存。本地关键字和当前文件符号会即时显示为列表，AI 则在短暂停顿后以流式内联建议显示；AI 建议可见时按 `Tab` 接受，否则 `Tab` 交给本地补全或缩进。底部终端可直接操作项目，也可唤起系统终端。AI Shell 默认关闭，每次开启都要先阅读风险警告并再次确认，默认还会逐条确认命令。
...
| 文本与代码 | TXT、JSON、YAML 和常见源码格式 | IDE 编辑、保存和可选 AI 代码补全 |
...
- 普通对话中的 AI 也可以创建、追加或替换项目内受支持的代码/文本文件，仍然必须先展示差异并由用户确认。
- “AI 操作”保存已接受写入的事务记录。只要文件之后没有被手工修改，就可以安全撤销整次多文件操作。

### IDE、终端与 AI Shell

- 左侧 `IDE` 入口把普通项目文件夹直接作为代码工作区；切换后右侧 AI 会话、当前模型和上下文范围不变。
- 代码文件在 IDE 文件树中单击只选择，双击才打开；编辑器支持跟随主题的语法高亮、高对比度光标、`Cmd/Ctrl + S` 保存，以及独立的代码自动保存开关和间隔。
- 双通道补全保留即时的本地关键字、当前文件变量和声明列表；AI 使用轻量前后缀和文件符号上下文，在有效代码或其后的新行以流式 ghost text 显示。AI 补全可单独配置服务商、模型和生成长度；建议可见时按 `Tab` 接受，继续输入或移动光标会取消旧请求并刷新建议。
- “当前内容 / 当前文档 / 当前项目”会识别当前代码文件；发送时冻结尚未保存的代码缓冲区，AI 文件预览仍会校验磁盘版本，避免绕过冲突检查。
- 内置终端运行在独立的主进程 PTY 中；渲染器没有 Node.js 权限。也可以从项目路径唤起系统终端。
