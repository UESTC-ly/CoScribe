...
| 系统 | 安装包 | 适用范围 |
| --- | --- | --- |
| macOS | `CoScribe-2.1.0-arm64.dmg` | Apple Silicon，macOS 13+ |
| macOS | `CoScribe-2.1.0-arm64-mac.zip` | Apple Silicon 免安装压缩包 |

v2.1.0 继续集中做好 Apple Silicon macOS。本项目目前没有 Apple Developer ID 签名，安装包可正常使用，但首次启动需要手动确认来源。
...
- 内置可信插件中心提供计划与日程、每日笔记与模板、闪卡与间隔复习、双向链接、性能诊断；插件按需加载，启用前明确列出权限。
...
- 项目始终是普通文件夹；CoScribe 只在根目录的 `.vibeknowledge/` 保存会话、布局、批注、OCR 缓存、可重建知识索引、AI 操作记录和插件本地数据。
...
- v2.1.0 只构建和验证 Apple Silicon macOS；本地语音识别暂不支持 Intel Mac 或 Windows。
...
- Audited built-in plugins provide Planner, Daily Notes and templates, spaced-repetition flashcards, backlinks, and on-demand diagnostics. Each plugin is lazy-loaded and shows its permissions before activation.
