...
> `v4.0.0` 开发分支新增完整 IDE 工作区：左侧 IDE 入口只切换中央工作区，AI 侧栏保持不变；代码文件单击选择、双击打开，可编辑保存并使用已配置模型补全。底部终端可直接操作项目，也可唤起系统终端。AI Shell 默认关闭，每次开启都要先阅读风险警告并再次确认，默认还会逐条确认命令。
...
- 代码文件在 IDE 文件树中单击只选择，双击才打开；编辑器支持语法高亮、`Cmd/Ctrl + S` 保存和 `Cmd/Ctrl + Space` AI 补全。
- “当前内容 / 当前文档 / 当前项目”会识别当前代码文件；发送时冻结尚未保存的代码缓冲区，AI 文件预览仍会校验磁盘版本，避免绕过冲突检查。
- 内置终端运行在独立的主进程 PTY 中；渲染器没有 Node.js 权限。也可以从项目路径唤起系统终端。
- AI Shell 与普通终端分离且默认关闭。设置只允许“请求开启”；每次实际开启都必须经过两次原生风险确认，授权只驻留内存并绑定当前项目，关闭终端、项目或应用即撤销。
