...
- 代码文件在 IDE 文件树中单击只选择，双击才打开；编辑器支持语法高亮、`Cmd/Ctrl + S` 保存和双通道补全。本地关键字、当前文件变量和声明会即时列出；AI 使用轻量前后缀和文件符号上下文，以流式 ghost text 显示。AI 建议可见时按 `Tab` 接受，继续输入或移动光标会取消旧请求并刷新建议。
