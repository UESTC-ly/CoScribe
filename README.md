...
> `v4.0.0` 开发分支新增完整 IDE 工作区：左侧 IDE 入口只切换中央工作区，AI 侧栏保持不变；代码文件单击选择、双击打开，可编辑保存并在输入停顿后显示 AI 内联补全，按 `Tab` 接受。底部终端可直接操作项目，也可唤起系统终端。AI Shell 默认关闭，每次开启都要先阅读风险警告并再次确认，默认还会逐条确认命令。
...
- 代码文件在 IDE 文件树中单击只选择，双击才打开；编辑器支持语法高亮、`Cmd/Ctrl + S` 保存和自动 AI 内联补全。输入停顿后会显示建议，按 `Tab` 接受；继续输入或移动光标会基于新代码刷新建议。
