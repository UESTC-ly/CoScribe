...
> 核对基线：2026-08-11，`package.json` 版本 `4.0.5`。本目录以当前源码、类型契约、测试配置和发布工作流为事实源；历史 Release 结果只在明确标注时使用。

CoScribe 是一个本地优先的 Electron 桌面工作台：普通文件夹就是项目，用户可以阅读资料、维护 Markdown 笔记、使用 AI 协作，并在 v4.0.5 的 IDE 工作区编辑代码和运行受控终端。
