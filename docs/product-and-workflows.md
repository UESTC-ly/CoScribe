...
- 双通道补全不会移除本地语言候选：本地关键字与当前文件声明会即时显示为列表；AI 在有效代码输入或其后的新行短暂停顿后流式显示内联建议。AI 建议可见时按 `Tab` 接受，否则交给本地补全或缩进；继续输入、移动光标或选择文本会取消旧 AI 请求并依据新上下文刷新。
- AI 补全可独立选择服务商、模型和简短/标准/长三档输出长度，不必跟随当前对话模型。
