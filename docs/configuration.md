...
- 默认上下文范围：当前内容、当前文档、当前项目或模型通用知识。旧版本保存的“选区”默认值会迁移为“当前内容”；存在文件或网页选区时，“当前内容”优先只使用选区。
- 会话上下文窗口和输出预留。
- 自动压缩早期请求快照。
- 是否允许模型使用通用知识。
...
“自动保存代码文件”默认开启，与 Markdown 自动保存开关和间隔互相独立。开启后，代码缓冲区在停止输入达到设定间隔后保存；关闭后只能通过编辑器保存按钮、`Cmd/Ctrl+S` 或关闭流程中的明确保存动作写盘。保存仍携带预期修改时间，磁盘文件被外部修改时不会静默覆盖。

编辑器的关键字和当前文件符号候选完全在 Renderer 本地生成，不需要 Provider 配置。当前版本不会在用户输入代码时自动请求 AI 模型。右侧 AI 对代码文件的创建和修改继续使用普通对话 Provider，并遵守差异预览、用户确认和主进程写入校验。
