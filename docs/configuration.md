...
“启用 AI 代码补全”默认开启，只影响编辑器发起的远程 AI 补全请求，不影响 IDE、本地关键字/当前文件符号补全、文件保存或普通 AI 对话。AI 补全使用短暂防抖、轻量前后缀和文件符号上下文；有效代码后的新行也会触发建议，新输入、光标移动或选区变化会取消过时请求。

代码补全可以独立选择已配置的服务商、模型和单次生成长度。服务商留空时跟随当前对话服务商，模型留空时使用该服务商默认模型；“简短 / 标准 / 长”分别限制在约 256 / 512 / 1,024 output tokens。密钥仍复用所选服务商的 `safeStorage` 配置，不会复制到 renderer。
